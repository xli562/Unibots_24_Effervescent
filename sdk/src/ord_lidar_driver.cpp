...
#define DEFAULT_TIMEOUT (2000)
					...
					if (temp_data.vailtidy_point_num + parsed_data_.point_num < POINT_CIRCLE_MAX_SIZE)
					{
						...
						for (int i = 0; i < POINT_PER_PACK; i++)
						{
							if ((last_angle - temp_data.data[temp_data.vailtidy_point_num + i].angle) > 1)
							{
								...
								if (full_scan_data_.data[0].angle > 5)
								{
	...
	bool OrdlidarDriver::SetRotationSpeed(int speed)
	{
		...
		request.data[2] = xor_check((uint8_t*)&request, HEAD_LEN + request.payload_len);
		request.data[3] = 0x31;
		request.data[4] = 0xF2;
	...
	bool OrdlidarDriver::GetFirmwareVersion(std::string &firmware_version) const
	{
		firmware_version = "V1.1.0"; // unuse
		return false;
	}
	...
	bool OrdlidarDriver::Activate(void)
	{
		...
		request.data[0] = 0X81;
		request.data[1] = xor_check((uint8_t*)&request, HEAD_LEN + request.payload_len);
		request.data[2] = 0x31;
		request.data[3] = 0xF2;
