...
while True:
    ...
    if (chassis.event_handler.timeout_flag) and not(chassis.event_handler.reset_flag):
        x_base = -100
        y_base = -100
        while abs(x_base) > 45 or abs(y_base) > 20:
            if abs(y_base) > 20:
                move('b', check_obstacle=distance_to_wall)
                base = chassis.find_base_no_turn_version()
                x_base, y_base = base[0], base[1]
                if chassis.event_handler.reset_flag and chassis.event_handler.timeout_flag:
                    chassis.stop()
                    break
            if x_base > 45:
                move('l', check_obstacle=distance_to_wall)
                base = chassis.find_base_no_turn_version()
                x_base, y_base = base[0], base[1]
                if chassis.event_handler.reset_flag and chassis.event_handler.timeout_flag:
                    chassis.stop()
